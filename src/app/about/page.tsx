...
const SKILLS = [
  ...
  { name: "Rust / C++",           level: 85 },
  { name: "Network Engineering",  level: 78 },
  { name: "Security Architecture",level: 81 },
  { name: "Data Engineering",     level: 74 },
  { name: "Technical Writing",    level: 95 },
];

const TIMELINE = [
  { year: "2023", event: "Mulai menulis TERMA//LOG sebagai jurnal teknis pribadi." },
  { year: "2024", event: "Bergabung dengan tim infrastruktur di perusahaan SaaS skala menengah." },
  { year: "2024", event: "Artikel pertama viral — lebih dari 40 ribu pembaca dalam 3 hari." },
  { year: "2025", event: "Meraih sertifikasi CKA (Certified Kubernetes Administrator)." },
  { year: "2026", event: "Merilis ulang blog dengan desain baru dan arsip terstruktur." },
];

const CONTACT = [
  { label: "EMAIL",   value: "nao@termalog.dev",          href: "mailto:nao@termalog.dev" },
  { label: "GITHUB",  value: "github.com/nao-termalog",   href: "https://github.com" },
  { label: "RSS",     value: "termalog.dev/rss.xml",      href: "/rss.xml" },
];
      ...
      <div className="relative z-10 px-8 py-10 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-px bg-[#bbb] border border-[#bbb] mb-10">
          ...
          <div className="bg-[#1a1a1a] md:w-48 shrink-0 flex flex-col items-center justify-center p-8 relative">
            ...
            <span className="mt-3 text-[9px] tracking-[0.2em] text-[#555]">UNIT-AUTHOR</span>
          </div>

          {/* Bio */}
          <div className="bg-[#c4c4c4] p-7 flex-1">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-[9px] tracking-[0.25em] text-[#888]">PENULIS</span>
              <span className="h-px flex-1 bg-[#bbb]" />
              <span className="text-[9px] tracking-[0.2em] text-[#e8c830]">AKTIF</span>
            </div>
            <h2 className="text-2xl font-black tracking-tight uppercase mb-1">Nao Termaguchi</h2>
            <p className="text-[11px] tracking-[0.2em] text-[#777] mb-5 uppercase">
              Insinyur Sistem · Penulis Teknis · Pengamat Infrastruktur
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <span className="text-[9px] tracking-[0.3em] text-[#888]">{"// KOMPETENSI"}</span>
              <span className="h-px flex-1 bg-[#bbb]" />
            </div>
              ...
              {SKILLS.map((sk) => (
                <div key={sk.name}>
                  <div className="flex justify-between mb-1">
                    <span className="text-[10px] tracking-[0.15em] text-[#444] uppercase">{sk.name}</span>
                    <span className="text-[10px] font-black text-[#e8c830]">{sk.level}</span>
                  </div>
                  <div className="h-[3px] bg-[#bbb]">
                    <div
                      className="h-[3px] bg-[#1a1a1a] transition-all duration-700"
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <span className="text-[9px] tracking-[0.3em] text-[#888]">{"// KRONOLOGI"}</span>
              <span className="h-px flex-1 bg-[#bbb]" />
            </div>
            <div className="flex flex-col relative">
              {/* vertical line */}
              <div className="absolute left-[26px] top-0 bottom-0 w-px bg-[#bbb]" />

              {TIMELINE.map((item, i) => (
                <div key={i} className="flex gap-4 mb-5 relative">
                  {/* dot */}
                  <div className="shrink-0 w-[13px] h-[13px] border-2 border-[#1a1a1a] bg-[#d0d0d0] mt-[2px] z-10 ml-[20px]" />
                  <div>
                    <span className="text-[9px] tracking-[0.3em] text-[#e8c830] block mb-1">{item.year}</span>
                    <p className="text-[11px] text-[#555] leading-relaxed">{item.event}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-[9px] tracking-[0.3em] text-[#888]">{"// KONTAK"}</span>
            <span className="h-px flex-1 bg-[#bbb]" />
          </div>
        ...
        <div className="border border-[#bbb] bg-[#c4c4c4] px-6 py-5 relative">
          ...
          <span className="text-[9px] tracking-[0.25em] text-[#888] block mb-2">{"// CATATAN"}</span>
          <p className="text-[11px] text-[#666] leading-relaxed">
            Semua opini, analisis, dan kesalahan di blog ini adalah milik saya sendiri
            dan tidak mencerminkan pandangan pemberi kerja manapun. Kode dan konten
            tersedia bebas kecuali disebutkan sebaliknya. Jika Anda menemukan
            kesalahan teknis, beri tahu saya.
          </p>
        </div>
